...
完整的端到端运行示例见 [`examples/`](examples/) 目录：

- [王亿博鲜活烧烤（唐家湾海景露台店）](examples/wangyibo-bbq/) —— 从商户原始信息输入，到符合腾讯元宝检索机制 + Princeton GEO 三大策略的公众号文章、腾讯新闻稿、metadata 全套输出。每个输出文件末尾附「GEO 策略落地说明表」，逐条对应策略落点。
